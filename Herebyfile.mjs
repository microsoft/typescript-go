...
export const depaware = task({
    ...
    run: async () => {
        ...
        const root = "github.com/microsoft/typescript-go/cmd/tsgo";
        ...
        for (const goos of geese) {
            ...
            })`go list -deps -f ${format} ./cmd/tsgo`;
        ...
        const stdout = lines.filter(l => !/^\s+github\.com\/microsoft\/typescript-go/.test(l)).join("\n").trim() + "\n";
        await fs.promises.writeFile(path.join(__dirname, "cmd", "tsgo", "depaware.txt"), stdout);
    },
});
