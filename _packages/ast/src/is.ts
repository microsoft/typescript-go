...
export function isTypeNode(node: Node): node is TypeNode {
    return (node.kind >= SyntaxKind.FirstTypeNode && node.kind <= SyntaxKind.LastTypeNode) || node.kind === SyntaxKind.JSDocAllType || node.kind === SyntaxKind.JSDocNullableType || node.kind === SyntaxKind.JSDocNonNullableType || node.kind === SyntaxKind.JSDocOptionalType || node.kind === SyntaxKind.JSDocVariadicType || node.kind === SyntaxKind.JSDocSignature;
}
