...
describe("Program - emitToString", () => {
    test("emitToString single file produces JS output", async () => {
        const { api, fs } = spawnAPIWithFS({
        ...
        try {
            const snapshot = await api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            const result = await project.program.emitToString("/src/a.ts");
    ...
    test("emit multiple files produces output for each", async () => {
        ...
        try {
            const snapshot = await api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            const result = await project.program.emitToString(["/src/a.ts", "/src/b.ts"]);
            assert.equal(result.emitSkipped, false);
            const jsFiles = result.outputFiles.filter(f => f.fileName.endsWith(".js"));
            assert.equal(jsFiles.length, 2);
            assert.deepEqual(jsFiles.map(f => f.fileName), ["/src/a.js", "/src/b.js"]);
    ...
    test("emit single file respects emitOnly", async () => {
        ...
        try {
            ...
            const all = await project.program.emitToString("/src/a.ts");
            ...
            const jsOnly = await project.program.emitToString("/src/a.ts", EmitOnly.OnlyJs);
            ...
            const dtsOnly = await project.program.emitToString("/src/a.ts", EmitOnly.OnlyDts);
...
describe("Program - emit", () => {
    ...
    test("emit only dts", async () => {
        ...
        try {
            const snapshot = await api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            const result = await project.program.emit(undefined, EmitOnly.OnlyDts);
            assert.deepEqual(result, {
                diagnostics: [],
                emitSkipped: false,
                emittedFiles: [
    ...
    test("emit only js", async () => {
        ...
        try {
            const snapshot = await api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            const result = await project.program.emit(undefined, EmitOnly.OnlyJs);
            assert.deepEqual(result, {
                diagnostics: [],
                emitSkipped: false,
                emittedFiles: [
    ...
    test("emit only for one source file", async () => {
        const fs = createVirtualFileSystem({ ...files });
        ...
        try {
            const snapshot = await api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            const result = await project.program.emit("/src/index.ts");
            assert.deepEqual(result, {
                diagnostics: [],
                emitSkipped: false,
                emittedFiles: [
    ...
    test("emit selected files and an empty selection", async () => {
        const { api, fs } = spawnAPIWithFS({ ...files });
        try {
            const snapshot = await api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;

            const selected = await project.program.emit(["/src/testing.ts"]);
            assert.deepEqual(selected.emittedFiles, [
                "/dist/src/testing.js",
                "/dist/src/testing.d.ts",
            ]);
            assert.equal(fs.readFile?.("/dist/src/index.js"), undefined);
            assert.equal(fs.readFile?.("/dist/src/testing.js"), `export const y = 'typescript';\n`);

            const empty = await project.program.emit([]);
    ...
    test("emit writes directly to the host filesystem when no VFS is configured", async () => {
        ...
        try {
            const snapshot = await api.updateSnapshot({ openProject: config });
    ...
    test("emit rejects unknown files and invalid emitOnly values", async () => {
        ...
        try {
            ...
            let error: unknown;
            try {
                await project.program.emit("/src/missing.ts");
            }
            catch (e) {
                error = e;
            }
            ...
            error = undefined;
            try {
                await project.program.emitToString(undefined, 999 as EmitOnly);
            }
            catch (e) {
                error = e;
            }
