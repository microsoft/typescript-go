...
describe("TypeParameter - getConstraint / getDefault", () => {
    test("returns the constraint and default of a type parameter", () => {
        ...
        try {
            const snapshot = api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            const symbol = project.checker.getSymbolAtPosition("/src/main.ts", src.indexOf("identity<"));
            assert.ok(symbol);
            const type = project.checker.getTypeOfSymbol(symbol);
            assert.ok(type);
            const sigs = project.checker.getSignaturesOfType(type, SignatureKind.Call);
    ...
    test("returns undefined when a type parameter has no constraint or default", () => {
...
describe("Type - isStructuredType", () => {
    test("is true for object types and false for primitives", () => {
        ...
        try {
            ...
            const iSymbol = project.checker.getSymbolAtPosition("/src/main.ts", src.indexOf("I {"));
            assert.ok(iSymbol);
            const iType = project.checker.getDeclaredTypeOfSymbol(iSymbol);
            assert.ok(iType);
            assert.ok(iType.isStructuredType());
            assert.ok(iType.flags & TypeFlags.StructuredType);

            const nSymbol = project.checker.getSymbolAtPosition("/src/main.ts", src.indexOf("n:"));
            assert.ok(nSymbol);
            const nType = project.checker.getTypeOfSymbol(nSymbol);
            assert.ok(nType);
...
describe("Type - TupleTypeReference", () => {
    test("getTarget on a tuple reference yields the tuple target", () => {
        const src = `export const tup: [number, string] = [1, "a"];`;
        const api = spawnAPI({
            "/tsconfig.json": JSON.stringify({ compilerOptions: { strict: true } }),
            "/src/main.ts": src,
        ...
        try {
            const snapshot = api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            const symbol = project.checker.getSymbolAtPosition("/src/main.ts", src.indexOf("tup"));
            assert.ok(symbol);
            const type = project.checker.getTypeOfSymbol(symbol);
            assert.ok(type);
            assert.equal(project.checker.isTupleType(type), true);
            ...
            const target = tupleRef.getTarget();
