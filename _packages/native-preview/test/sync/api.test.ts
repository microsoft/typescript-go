...
describe("Program - emit", () => {
    test("emit single file produces JS output", () => {
        const api = spawnAPI({
        ...
        try {
            const snapshot = api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            const result = project.program.emit("/src/a.ts");
    ...
    test("emit multiple files produces output for each", () => {
        ...
        try {
            const snapshot = api.updateSnapshot({ openProject: "/tsconfig.json" });
            const project = snapshot.getProject("/tsconfig.json")!;
            const result = project.program.emit(["/src/a.ts", "/src/b.ts"]);
            assert.equal(result.emitSkipped, false);
    ...
    test("emit single file respects emitOnly", () => {
        ...
        try {
            ...
            const all = project.program.emit("/src/a.ts");
            ...
            const jsOnly = project.program.emit("/src/a.ts", EmitOnly.OnlyJs);
            ...
            const dtsOnly = project.program.emit("/src/a.ts", EmitOnly.OnlyDts);
