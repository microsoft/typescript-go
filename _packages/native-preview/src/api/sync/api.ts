...
export class Signature {
    ...
    private fetchTypes(method: string): readonly Type[] {
        const data = this.client.apiRequest<TypeResponse[] | null>(method, { snapshot: this.snapshotId, signature: this.id });
        return data ? data.map(d => this.objectRegistry.getOrCreateType(d) as Type) : [];
    }

    private fetchSymbol(handle: number | undefined, method: string): Symbol | undefined {
        if (handle === undefined) return undefined;
        const cached = this.objectRegistry.getSymbol(handle);
        if (cached) return cached;
        const data = this.client.apiRequest<SymbolResponse | null>(method, { snapshot: this.snapshotId, signature: this.id });
        return data ? this.objectRegistry.getOrCreateSymbol(data) : undefined;
    }

    private fetchSymbols(method: string): readonly Symbol[] {
        const data = this.client.apiRequest<SymbolResponse[] | null>(method, { snapshot: this.snapshotId, signature: this.id });
        return data ? data.map(d => this.objectRegistry.getOrCreateSymbol(d)) : [];
    }
    ...
    getTarget(): Signature | undefined {
        ...
        const data = this.client.apiRequest<SignatureResponse | null>("getTargetOfSignature", { snapshot: this.snapshotId, signature: this.id });
        return data ? this.objectRegistry.getOrCreateSignature(data) : undefined;
    }
