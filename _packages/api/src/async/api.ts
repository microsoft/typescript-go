...
export class API implements BaseAPI<true> {
    private server: Killable;
    private client: Client;

    constructor(options: APIOptions) {
        this.client = new Client(this.server = startLSPServer(options.tsserverPath, options.cwd ?? process.cwd()));
    }
...
export class Project extends BaseProject<true> {
    ...
    async getSourceFile(fileName: string): Promise<SourceFileNode | undefined> {
