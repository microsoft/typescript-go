...
// Minimized reproduction of a stack overflow in tsgo when using recursive
// generic functions with complex conditional/mapped types (from type-fest).
// The crash only occurs in multi-file scenarios where the types are imported.
// See: https://github.com/microsoft/typescript-go/issues/1093
//
// All types below are inlined from type-fest v4.38.0 (the version that
// introduced ApplyDefaultOptions, which triggers the crash).
...
// This pattern causes a stack overflow in tsgo but compiles with tsc.
