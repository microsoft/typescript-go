...
let super_covariant: Covariant<unknown>;
let sub_covariant: Covariant<string>;
type Contravariant<in T> = {
    f: (x: T) => void;
};
let super_contravariant: Contravariant<unknown>;
let sub_contravariant: Contravariant<string>;
type Invariant<in out T> = {
    f: (x: T) => T;
};
let super_invariant: Invariant<unknown>;
let sub_invariant: Invariant<string>;
type T10<out T> = T;
type T11<in T> = keyof T;
type T12<out T, out K extends keyof T> = T[K];
type T13<in out T> = T[keyof T];
...
const pu: Parent<unknown>;
const notString: Parent<string>;
class StateNode<TContext, in out TEvent extends {
    type: string;
}> {
    _storedEvent: TEvent;
    _action: ActionObject<TEvent>;
...
function createMachine<TEvent extends {
    type: string;
}>(action: ActionObject<TEvent>): StateNode<any, any>;
function interpret<TContext>(machine: StateNode<TContext, any>): void;
const machine: StateNode<any, any>;
const qq: ActionObject<{
    type: "PLAY";
    value: number;
}>;
let Anon: {
