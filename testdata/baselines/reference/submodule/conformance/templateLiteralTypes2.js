...
function ft1<T extends string>(s: string, n: number, u: 'foo' | 'bar' | 'baz', t: T): void;
function ft2(s: string): string;
function ft10(s: string): void;
function ft11(s: string, cond: boolean): void;
function ft12(s: string): void;
function widening<T>(x: T): T;
function nonWidening<T extends string | number | symbol>(x: T): T;
function ft13(s: string, cond: boolean): void;
type T0 = string | `${number}px`;
function ft14(t: `foo${number}`): void;
function g1<T>(x: T): T;
function g2<T extends string>(x: T): T;
function ft20(s: string): void;
function takesLiteral<T extends string>(literal: T): T extends `foo.bar.${infer R}` ? R : unknown;
const t1: "baz";
const id2 = "foo.bar.baz";
const t2: "baz";
...
const id4: string;
const t4: unknown;
const someUnion: 'abc' | 'def' | 'ghi';
const t5: "abc" | "def" | "ghi";
const pixelValue: number;
type PixelValueType = `${number}px`;
const pixelString: PixelValueType;
const pixelStringWithTemplate: PixelValueType;
function getCardTitle(title: string): `test-${string}`;
const interpolatedStyle: {
    rotate: number;
};
function C2(transform: "-moz-initial" | (string & {})): number;
