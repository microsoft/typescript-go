...
let f1: (x: string, ...args: [string] | [number, boolean]) => void;
let f2: (x: string, y: string) => void;
let f3: (x: string, y: number, z: boolean) => void;
let f4: (...args: [string, string] | [string, number, boolean]) => void;
const t1: [string] | [number, boolean];
const t2: readonly [string] | [number, boolean];
const t3: [string] | readonly [number, boolean];
const t4: readonly [string] | readonly [number, boolean];
interface CoolArray<E> extends Array<E> {
    hello: number;
}
function foo<T extends any[]>(cb: (...args: T) => void): void;
function bar<T extends any[]>(...args: T): T;
let a: [number, number];
let b: CoolArray<number>;
function baz<T>(...args: CoolArray<T>): void;
const ca: CoolArray<number>;
function hmm<A extends [] | [number, string]>(...args: A): void;
function foo2(...args: string[] | number[]): void;
let x2: ReadonlyArray<string>;
type RestParams = [y: string] | [y: number];
type Signature = (x: string, ...rest: RestParams) => void;
type MergedParams = Parameters<Signature>;
let ff1: (...rest: [string, string] | [string, number]) => void;
let ff2: (x: string, ...rest: [string] | [number]) => void;
function ff3<A extends unknown[]>(s1: (...args: [x: string, ...rest: A | [number]]) => void, s2: (x: string, ...rest: A | [number]) => void): void;
