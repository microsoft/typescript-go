...
type IfEquals<X, Y, A, B> = (<T>() => T extends X ? 1 : 2) extends <T>() => (T extends Y ? 1 : 2) ? A : B;
const x1: <T>() => (T extends infer U extends number ? 1 : 0);
function f1(): <T>() => (T extends infer U extends number ? 1 : 0);
type ExpectNumber<T extends number> = T;
const x2: <T>() => (T extends ExpectNumber<infer U> ? 1 : 0);
function f2(): <T>() => (T extends ExpectNumber<infer U> ? 1 : 0);


//// [DtsFileErrors]


inferTypesWithExtends1.d.ts(146,1): error TS1046: Top-level declarations in .d.ts files must start with either a 'declare' or 'export' modifier.


==== inferTypesWithExtends1.d.ts (1 errors) ====
    type X1<T extends any[]> = T extends [infer U extends string] ? ["string", U] : T extends [infer U extends number] ? ["number", U] : never;
    type X1_T1 = X1<["a"]>;
    type X1_T2 = X1<[1]>;
    type X1_T3 = X1<[object]>;
