...
//// [inferTypes2.d.ts]
export function foo<T>(obj: T): T extends () => infer P ? P : never;
export function bar<T>(obj: T): T extends () => infer P ? P : never;
export type BadNested<T> = {
    x: T extends number ? T : string;
};
export function foo2<T>(obj: T): T extends {
    [K in keyof BadNested<infer P>]: BadNested<infer P>[K];
} ? P : never;
export function bar2<T>(obj: T): T extends {
