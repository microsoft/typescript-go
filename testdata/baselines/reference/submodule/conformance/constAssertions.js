...
let vc1: "abc";
let a1: readonly [];
let a2: readonly [1, 2, 3];
let a3: readonly [10, 'hello', true];
let a4: readonly [1, 2, 3];
let a5: number[];
let a6: readonly number[];
let a7: number[];
let a8: readonly ["abc", ...number[]];
let a9: (number | "abc")[];
let d: {
    [x: string]: string;
};
...
let p2: -10;
let p3: readonly [10];
let p4: readonly [readonly [readonly [readonly [10]]]];
let x1: {
    readonly x: 10;
...
let t3: "foo-bar";
let t4: "(foo)-(bar)";
function ff1(x: 'foo' | 'bar', y: 1 | 2): "bar-1" | "bar-2" | "foo-1" | "foo-2";
function ff2<T extends string, U extends string>(x: T, y: U): `${T}-${U}`;
const ts1: "foo-bar";
const ts2: "foo-0" | "foo-1";
const ts3: "bottom-left" | "bottom-right" | "top-left" | "top-right";
function ff3(x: 'foo' | 'bar', y: object): `bar${string}` | `foo${string}`;
type Action = "verify" | "write";
type ContentMatch = "match" | "nonMatch";
type Outcome = `${Action}_${ContentMatch}`;
function ff4(verify: boolean, contentMatches: boolean): "verify_match" | "verify_nonMatch" | "write_match" | "write_nonMatch";
function ff5(verify: boolean, contentMatches: boolean): "verify_match" | "verify_nonMatch" | "write_match" | "write_nonMatch";
function accessorNames<S extends string>(propName: S): readonly [`get-${S}`, `set-${S}`];
const ns1: readonly ["get-foo", "set-foo"];
interface Foo54374 {
    a: 1;
    b: 2;
}
