...
type T53 = Readonly<Partial<string[]>>;
type T54 = ReadWrite<Required<T53>>;
function unboxify<T>(x: Boxified<T>): T;
let x10: [Box<number>, Box<string>, ...Box<boolean>[]];
let y10: [number, string, ...boolean[]];
let x11: Box<number>[];
let y11: number[];
let x12: {
    a: Box<number>;
...
function nonpartial<T>(x: Partial<T>): T;
let x20: [number | undefined, string?, ...boolean[]];
let y20: [number, string, ...boolean[]];
let x21: (number | undefined)[];
let y21: number[];
let x22: {
    a: number | undefined;
    b?: string[];
...
function all<T extends any[]>(...values: T): Promise<Awaitified<T>>;
function f1(a: number, b: Promise<number>, c: string[], d: Promise<string[]>): void;
function f2<T extends any[]>(a: Boxified<T>): void;
type ElementType<T> = T extends Array<infer U> ? U : never;
type Mapped<T> = {
    [K in keyof T]: T[K];
};
...
function acceptArray(arr: any[]): void;
function mapArray<T extends any[]>(arr: T): Mapped<T>;
function acceptMappedArray<T extends any[]>(arr: T): void;
type Unconstrained<T> = ElementType<Mapped<T>>;
...
mappedTypesArraysTuples.d.ts(39,1): error TS1046: Top-level declarations in .d.ts files must start with either a 'declare' or 'export' modifier.


==== mappedTypesArraysTuples.d.ts (1 errors) ====
    type Box<T> = {
        value: T;
    };
    type Boxified<T> = {
