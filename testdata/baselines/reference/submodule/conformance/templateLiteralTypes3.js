...
type T10 = Foo1<`**${'a' | 'b' | 'c'}**`>;
type T11 = Foo1<`**${boolean}**${boolean}**`>;
function foo1<V extends string>(arg: `*${V}*`): V;
function f1<T extends string>(s: string, n: number, b: boolean, t: T): void;
...
type T23 = Parts<`*${number}*${string}*${bigint}*`>;
function f2(): void;
function f3<T extends string>(s: string, n: number, b: boolean, t: T): void;
function f4<T extends number>(s: string, n: number, b: boolean, t: T): void;
type A<T> = T extends `${infer U}.${infer V}` ? U | V : never;
type B = A<`test.1024`>;
...
type Templated = `${string} ${string}`;
const value1: string;
const templated1: Templated;
const value2 = "abc";
const templated2: Templated;
type Prefixes = "foo" | "bar";
type AllPrefixData = "foo:baz" | "bar:baz";
type PrefixData<P extends Prefixes> = `${P}:baz`;
interface ITest<P extends Prefixes, E extends AllPrefixData = PrefixData<P>> {
...
function chain<F extends keyof Schema>(field: F | `${F}.${F}`): void;
function ff1(x: `foo-${string}`, y: `${string}-bar`, z: `baz-${string}`): void;
function ff2<T extends string>(x: `foo-${T}`, y: `${T}-bar`, z: `baz-${T}`): void;
function ff3(x: string, y: `foo-${string}` | 'bar'): void;
function ff4(x: string, y: `foo-${string}`): void;
type Action = {
    type: `${string}_REQUEST`;
} | {
...
type DotString = `${string}.${string}.${string}`;
function noSpread<P extends DotString>(args: P[]): P;
function spread<P extends DotString>(...args: P[]): P;
function ft1<T extends string>(t: T, u: Uppercase<T>, u1: Uppercase<`1.${T}.3`>, u2: Uppercase<`1.${T}.4`>): void;
type Boom = 'abc' | 'def' | `a${string}` | Lowercase<string>;
function a<T extends {
    id: string;
}>(): void;
