...
type addQuestionMarks<T extends object, R extends keyof T = requiredKeys<T>> = Pick<Required<T>, R> & Partial<T>;
type identity<T> = T;
type flatten<T> = identity<{
    [k in keyof T]: T[k];
}>;
export abstract class ZodType<Output = any> {
    readonly _output: Output;
}
...
export type objectOutputType<Shape extends ZodRawShape> = flatten<addQuestionMarks<baseObjectOutputType<Shape>>>;
export type ZodRawShape = {
    [k: string]: ZodTypeAny;
};
export const buildSchema: <V extends string>(version: V) => objectOutputType<{
    version: ZodLiteral<V>;
}>;
type evaluate<t> = {
    [k in keyof t]: t[k];
