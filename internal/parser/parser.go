...
func (p *Parser) parseModuleDeclaration(pos int, hasJSDoc bool, modifiers *ast.ModifierList) *ast.Statement {
	if p.token == ast.KindGlobalKeyword {
		...
		return p.parseModuleOrNamespaceDeclaration(pos, hasJSDoc, modifiers, false /*nested*/, false /*isIllegalModuleKeyword*/)
	} else {
		...
		return p.parseModuleOrNamespaceDeclaration(pos, hasJSDoc, modifiers, false /*nested*/, true /*isIllegalModuleKeyword*/)
	}
}
...
func (p *Parser) parseModuleOrNamespaceDeclaration(pos int, hasJSDoc bool, modifiers *ast.ModifierList, nested bool, isIllegalModuleKeyword bool) *ast.Node {
	saveHasAwaitIdentifier := p.statementHasAwaitIdentifier
	var name *ast.Node
	if nested {
	...
	if p.parseOptional(ast.KindDotToken) {
		...
		body = p.parseModuleOrNamespaceDeclaration(p.nodePos(), false /*hasJSDoc*/, implicitModifiers, true /*nested*/, isIllegalModuleKeyword)
	} else {
		body = p.parseModuleBlock()
	}
