...
func (s *Session) HandleRequest(ctx context.Context, method string, params json.Value) (any, error) {
	...
	case string(MethodGetDefaultOfTypeParameter):
		return s.handleGetDefaultOfTypeParameter(ctx, parsed.(*CheckerTypeParams))
...
// handleGetDefaultOfTypeParameter returns the default type of a type parameter.
func (s *Session) handleGetDefaultOfTypeParameter(ctx context.Context, params *CheckerTypeParams) (*TypeResponse, error) {
	setup, err := s.setupChecker(ctx, params.Snapshot, params.Project)
	if err != nil {
		return nil, err
	}
