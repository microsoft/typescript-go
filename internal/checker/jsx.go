	...
	if !isJsxOpenFragment {
		if isJsxIntrinsicTagName(node.TagName()) {
			...
			if len(typeArguments) != 0 {
				...
				c.addDiagnostic(ast.NewDiagnostic(ast.GetSourceFileOfNode(node), node.TypeArgumentList().Loc, diagnostics.Expected_0_type_arguments_but_got_1, 0, len(typeArguments)))
			}
