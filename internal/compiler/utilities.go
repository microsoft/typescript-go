...
func visitDirectory(visited map[string]bool, path string, absolutePath string, depth int, useCaseSensitiveFileNames bool, host vfs.FS, getFileSystemEntries func(path string, host vfs.FS) FileSystemEntries, includeFileRegexes []*regexp2.Regexp, excludeRegex *regexp2.Regexp, results [][]string, includeDirectoryRegex *regexp2.Regexp) {
	...
	for _, current := range files {
		...
		if (extensions != nil && !tspath.FileExtensionIsOneOf(name, extensions)) || (excludeRegex != nil && must(excludeRegex.MatchString(absoluteName))) {
			continue
		}
		if includeFileRegexes == nil {
			results[0] = append(results[0], name)
		} else {
			includeIndex := core.FindIndex(includeFileRegexes, func(re *regexp2.Regexp) bool { return must(re.MatchString(absoluteName)) })
			if includeIndex != -1 {
	...
	for _, current := range directories {
		name := tspath.CombinePaths(path, current)
		absoluteName := tspath.CombinePaths(absolutePath, current)
		if (includeDirectoryRegex == nil || must(includeDirectoryRegex.MatchString(absoluteName))) && (excludeRegex == nil || !must(excludeRegex.MatchString(absoluteName))) {
			visitDirectory(visited, name, absoluteName, depth, useCaseSensitiveFileNames, host, getFileSystemEntries, includeFileRegexes, excludeRegex, results, includeDirectoryRegex)
		}
	}
}
...
func matchFiles(path string, extensions []string, excludes []string, includes []string, useCaseSensitiveFileNames bool, currentDirectory string, depth int, host vfs.FS, getFileSystemEntries func(path string, host vfs.FS) FileSystemEntries, realpath func(path string) string) []string {
	...
	for _, basePath := range patterns.basePaths {
		visitDirectory(visited, basePath, tspath.CombinePaths(currentDirectory, basePath), depth, useCaseSensitiveFileNames, host, getFileSystemEntries, includeFileRegexes, excludeRegex, results, includeDirectoryRegex)
	}
...
func tryGetTextOfPropertyName(name *ast.PropertyName) string {
	switch name.Kind {
	case ast.KindIdentifier:
	case ast.KindPrivateIdentifier:
		return name.Text()
	...
	case ast.KindComputedPropertyName:
		...
		return ""
	case ast.KindJsxNamespacedName:
		return name.Text() //getEscapedTextOfJsxNamespacedName(name);
	default:
...
func forEachPropertyAssignment(objectLiteral *ast.ObjectLiteralExpression, key string, callback func(property ast.PropertyAssignment) *ast.Node, key2 string) string {
