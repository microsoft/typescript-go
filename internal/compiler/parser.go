...
func (p *Parser) parseDeclaration() *Statement {
	...
	isAmbient := modifierList != nil && funcs.Some(modifierList.AsModifierList().modifiers, isDeclareModifier)
	if isAmbient {
		// !!! incremental parsing
...
func (p *Parser) parseClassDeclarationOrExpression(pos int, hasJSDoc bool, modifiers *Node, kind SyntaxKind) *Node {
	...
	typeParameters := p.parseTypeParameters()
	if modifiers != nil && funcs.Some(modifiers.AsModifierList().modifiers, isExportModifier) {
...
func hasAsyncModifier(modifiers *Node) bool {
	return modifiers != nil && funcs.Some(modifiers.AsModifierList().modifiers, isAsyncModifier)
}
