...
type ProjectReferenceFileMapper struct {
	...
	referencesInConfigFile   map[tspath.Path][]*tsoptions.ParsedCommandLine // Map of config file to its references
...
func (mapper *ProjectReferenceFileMapper) init(loader *fileLoader, rootTasks []*projectReferenceParseTask) {
	...
	mapper.referencesInConfigFile = make(map[tspath.Path][]*tsoptions.ParsedCommandLine, totalReferences)
	...
	mapper.referencesInConfigFile[loader.toPath(mapper.opts.Config.CompilerOptions().ConfigFilePath)] = loader.projectReferenceParseTasks.collect(rootTasks, func(task *projectReferenceParseTask, referencesInConfig []*tsoptions.ParsedCommandLine) *tsoptions.ParsedCommandLine {
		if task.resolved == nil {
			return nil
		}
		mapper.configToProjectReference[task.path] = task.resolved
		if mapper.opts.Config.ConfigFile != task.resolved.ConfigFile {
			mapper.referencesInConfigFile[task.path] = referencesInConfig
...
func (mapper *ProjectReferenceFileMapper) GetResolvedProjectReferences() []*tsoptions.ParsedCommandLine {
	return mapper.referencesInConfigFile[tspath.ToPath(mapper.opts.Config.CompilerOptions().ConfigFilePath, mapper.comparePathsOptions.CurrentDirectory, mapper.comparePathsOptions.UseCaseSensitiveFileNames)]
}
...
func (mapper *ProjectReferenceFileMapper) getOutputAndProjectReference(path tspath.Path) *tsoptions.OutputDtsAndProjectReference {
	result, ok := mapper.sourceToOutput[path]
...
func (mapper *ProjectReferenceFileMapper) getSourceAndProjectReference(path tspath.Path) *tsoptions.SourceAndProjectReference {
	result, ok := mapper.outputDtsToSource[path]
	return core.IfElse(ok, result, nil)
}

func (mapper *ProjectReferenceFileMapper) isSourceFromProjectReference(path tspath.Path) bool {
	if mapper.opts.canUseProjectReferenceSource() {
		return mapper.getOutputAndProjectReference(path) != nil
	}
	return false
}
