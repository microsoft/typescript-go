...
type FunctionBodyOrExpression = Node    // FunctionBody | Expression
type AccessExpression = Node            // PropertyAccessExpression | ElementAccessExpression
type DeclarationName = Node             // IdentifierName | PrivateIdentifier | StringLiteral | NumericLiteral | BigIntLiteral | NoSubstitutionTemplateLiteral | ComputedPropertyName | BindingIdentifier | BindingPattern | ElementAccessExpression
type ModuleName = Node                  // BindingIdentifier | StringLiteral
type ModuleExportName = Node            // IdentifierName | StringLiteral
type PropertyName = Node                // IdentifierName | StringLiteral | NoSubstitutionTemplateLiteral | NumericLiteral | ComputedPropertyName | PrivateIdentifier | BigIntLiteral
type ModuleBody = Node                  // ModuleBlock | ModuleDeclaration
type ForInitializer = Node              // Expression | VariableDeclarationList
type ModuleReference = Node             // IdentifierReference | QualifiedName | ExternalModuleReference
type NamedImportBindings = Node         // NamespaceImport | NamedImports
type NamedExportBindings = Node         // NamespaceExport | NamedExports
type MemberName = Node                  // IdentifierName | PrivateIdentifier
type EntityName = Node                  // IdentifierName | QualifiedName
type BindingName = Node                 // BindingIdentifier | BindingPattern
...
type JsxAttributeName = Node            // IdentifierName | JsxNamespacedName
...
type IdentifierNameNode = Node
type IdentifierReferenceNode = Node
type BindingIdentifierNode = Node
type LabelIdentifierNode = Node
...
type BodyBase struct {
	...
	body          *FunctionBodyOrExpression // FunctionBodyOrExpression. Optional, can be Expression only in arrow functions
...
type Identifier struct {
	ExpressionBase // TODO(rbuckton): IdentifierName, BindingIdentifier, and LabelIdentifier are not expressions
	FlowNodeBase
	text string
}

func (f *NodeFactory) newIdentifier(kind SyntaxKind, text string) *Node {
	assertKind(kind, SyntaxKindIdentifierName, SyntaxKindIdentifierReference, SyntaxKindBindingIdentifier, SyntaxKindLabelIdentifier)
	data := f.identifierPool.New()
	data.text = text
	return f.NewNode(kind, data)
}

func (f *NodeFactory) NewIdentifierName(text string) *Node {
	return f.newIdentifier(SyntaxKindIdentifierName, text)
}

func (f *NodeFactory) NewIdentifierReference(text string) *Node {
	return f.newIdentifier(SyntaxKindIdentifierReference, text)
}

func (f *NodeFactory) NewBindingIdentifier(text string) *Node {
	return f.newIdentifier(SyntaxKindBindingIdentifier, text)
}

func (f *NodeFactory) NewLabelIdentifier(text string) *Node {
	return f.newIdentifier(SyntaxKindLabelIdentifier, text)
}

func isIdentifierName(node *Node) bool {
	return node.kind == SyntaxKindIdentifierName
}

func isIdentifierReference(node *Node) bool {
	return node.kind == SyntaxKindIdentifierReference
}

func isBindingIdentifier(node *Node) bool {
	return node.kind == SyntaxKindBindingIdentifier
}

func isLabelIdentifier(node *Node) bool {
	return node.kind == SyntaxKindLabelIdentifier
}

func isIdentifier(node *Node) bool {
	return isIdentifierName(node) || isIdentifierReference(node) || isBindingIdentifier(node) || isLabelIdentifier(node)
}

// PrivateIdentifier
// TODO(rbuckton): Could we collapse this struct into `Identifier`?
...
type QualifiedName struct {
	...
	right *IdentifierNameNode // IdentifierNameNode
}

func (f *NodeFactory) NewQualifiedName(left *EntityName, right *IdentifierNameNode) *Node {
	assertEntityName(left)
	assertIdentifierName(right)
	data := &QualifiedName{}
	data.left = left
	data.right = right
...
type TypeParameterDeclaration struct {
	...
	DeclarationBase
	ModifiersBase
	name        *BindingIdentifierNode // BindingIdentifierNode
...
func (f *NodeFactory) NewTypeParameterDeclaration(modifiers *ModifierListNode, name *BindingIdentifierNode, constraint *TypeNode, defaultType *TypeNode) *Node {
	assertModifiersOpt(modifiers)
	assertBindingIdentifier(name)
	assertTypeNodeOpt(constraint)
	assertTypeNodeOpt(defaultType)
...
func (f *NodeFactory) NewDecorator(expression *LeftHandSideExpression) *Node {
	assertLeftHandSideExpression(expression)
	data := &Decorator{}
	data.expression = expression
...
func (f *NodeFactory) NewIfStatement(expression *Expression, thenStatement *Statement, elseStatement *Statement) *Node {
	assertExpression(expression)
	assertStatement(thenStatement)
	assertStatementOpt(elseStatement)
	data := &IfStatement{}
	data.expression = expression
...
func (f *NodeFactory) NewForStatement(initializer *ForInitializer, condition *Expression, incrementor *Expression, statement *Statement) *Node {
	...
	assertExpressionOpt(condition)
	assertExpressionOpt(incrementor)
	assertStatement(statement)
	data := &ForStatement{}
...
type BreakStatement struct {
	StatementBase
	label *LabelIdentifierNode // Optional
}

func (f *NodeFactory) NewBreakStatement(label *LabelIdentifierNode) *Node {
	assertNodeOpt(label, isLabelIdentifier)
	data := &BreakStatement{}
	data.label = label
...
func (f *NodeFactory) NewContinueStatement(label *LabelIdentifierNode) *Node {
	assertNodeOpt(label, isLabelIdentifier)
...
func (f *NodeFactory) NewReturnStatement(expression *Expression) *Node {
	assertExpressionOpt(expression)
...
func (f *NodeFactory) NewTryStatement(tryBlock *BlockNode, catchClause *CatchClauseNode, finallyBlock *BlockNode) *Node {
	assertBlock(tryBlock)
	assertNodeOpt(catchClause, isCatchClause)
	assertBlockOpt(finallyBlock)
	data := &TryStatement{}
...
type LabeledStatement struct {
	StatementBase
	label     *LabelIdentifierNode // LabelIdentifierNode
	statement *Statement           // Statement
}

func (f *NodeFactory) NewLabeledStatement(label *LabelIdentifierNode, statement *Statement) *Node {
	assertLabelIdentifier(label)
	assertStatement(statement)
...
// TODO(rbuckton): Should we introduce a new SyntaxKind for [BlockStatement] to differentiate from blocks used in
// try..catch? That would remove the need to look at parent pointers in utilities.isBlockStatement.
//
// [BlockStatement]: https://tc39.es/ecma262/#prod-BlockStatement

// TODO(rbuckton): Should we introduce a new SyntaxKind for [ClassStaticBlockBody] to match the spec? It would avoid
// parent pointer lookups in a few cases.
//
// [ClassStaticBlockBody]: https://tc39.es/ecma262/#prod-ClassStaticBlockBody

// TODO(rbuckton): We could also introduce [AsyncFunctionBody], [GeneratorBody], and [AsyncGeneratorBody], though there
// are likely diminishing returns for those cases.
//
// [AsyncFunctionBody]: https://tc39.es/ecma262/#prod-AsyncFunctionBody
// [GeneratorBody]: https://tc39.es/ecma262/#prod-GeneratorBody
// [AsyncGeneratorBody]: https://tc39.es/ecma262/#prod-AsyncGeneratorBody
...
func (f *NodeFactory) newBlock(kind SyntaxKind, statements []*Statement, multiline bool) *Node {
	assertKind(kind, SyntaxKindBlock, SyntaxKindFunctionBody)
	assertStatements(statements)
	data := &Block{}
	data.statements = statements
...
func (f *NodeFactory) NewBlock(statements []*Statement, multiline bool) *Node {
	return f.newBlock(SyntaxKindBlock, statements, multiline)
}

func (f *NodeFactory) NewFunctionBody(statements []*Statement, multiline bool) *Node {
	return f.newBlock(SyntaxKindFunctionBody, statements, multiline)
}
...
func isFunctionBody(node *Node) bool {
	return node.kind == SyntaxKindFunctionBody
}
...
func (f *NodeFactory) NewVariableDeclaration(name *BindingName, exclamationToken *TokenNode, typeNode *TypeNode, initializer *Expression) *Node {
	...
	assertTypeNodeOpt(typeNode)
	assertExpressionOpt(initializer)
...
func (f *NodeFactory) NewParameterDeclaration(modifiers *ModifierListNode, dotDotDotToken *TokenNode, name *BindingName, questionToken *TokenNode, typeNode *TypeNode, initializer *Expression) *Node {
	assertModifiersOpt(modifiers)
	assertDotDotDotTokenOpt(dotDotDotToken)
	assertBindingName(name)
...
func (f *NodeFactory) NewFunctionDeclaration(modifiers *ModifierListNode, asteriskToken *TokenNode, name *BindingIdentifierNode, typeParameters *TypeParameterListNode, parameters []*ParameterDeclarationNode, returnType *TypeNode, body *FunctionBodyNode) *Node {
	assertModifiersOpt(modifiers)
	assertAsteriskTokenOpt(asteriskToken)
	assertBindingIdentifierOpt(name)
	assertTypeParametersOpt(typeParameters)
	assertParameters(parameters)
	assertTypeNodeOpt(returnType)
	assertFunctionBodyOpt(body)
...
func (f *NodeFactory) NewClassDeclaration(modifiers *ModifierListNode, name *BindingIdentifierNode, typeParameters *TypeParameterListNode, heritageClauses []*HeritageClauseNode, members []*ClassElement) *Node {
	assertModifiersOpt(modifiers)
	assertBindingIdentifierOpt(name)
	assertTypeParametersOpt(typeParameters)
...
func (f *NodeFactory) NewClassExpression(modifiers *ModifierListNode, name *BindingIdentifierNode, typeParameters *TypeParameterListNode, heritageClauses []*HeritageClauseNode, members []*ClassElement) *Node {
	assertModifiersOpt(modifiers)
	assertBindingIdentifierOpt(name)
	assertTypeParametersOpt(typeParameters)
	assertNodes(heritageClauses, isHeritageClause)
...
func (f *NodeFactory) NewInterfaceDeclaration(modifiers *ModifierListNode, name *BindingIdentifierNode, typeParameters *TypeParameterListNode, heritageClauses []*HeritageClauseNode, members []*TypeElement) *Node {
	assertModifiersOpt(modifiers)
	assertBindingIdentifier(name)
	assertTypeParametersOpt(typeParameters)
	assertNodes(heritageClauses, isHeritageClause)
...
func (f *NodeFactory) NewTypeAliasDeclaration(modifiers *ModifierListNode, name *BindingIdentifierNode, typeParameters *TypeParameterListNode, typeNode *TypeNode) *Node {
	assertModifiersOpt(modifiers)
	assertBindingIdentifier(name)
	assertTypeParametersOpt(typeParameters)
...
func (f *NodeFactory) NewEnumDeclaration(modifiers *ModifierListNode, name *BindingIdentifierNode, members []*EnumMemberNode) *Node {
	assertModifiersOpt(modifiers)
...
func (f *NodeFactory) NewModuleDeclaration(modifiers *ModifierListNode, name *ModuleName, body *ModuleBody, flags NodeFlags) *Node {
	assertModifiersOpt(modifiers)
	assertNode(name, isBindingIdentifier, isStringLiteral)
...
func (f *NodeFactory) NewImportEqualsDeclaration(modifiers *ModifierListNode, isTypeOnly bool, name *BindingIdentifierNode, moduleReference *ModuleReference) *Node {
	assertModifiersOpt(modifiers)
	assertBindingIdentifier(name)
...
func (f *NodeFactory) NewImportDeclaration(modifiers *ModifierListNode, importClause *ImportClauseNode, moduleSpecifier *Expression, attributes *ImportAttributesNode) *Node {
	...
	assertExpression(moduleSpecifier)
...
func (f *NodeFactory) NewImportSpecifier(isTypeOnly bool, propertyName *ModuleExportName, name *BindingIdentifierNode) *Node {
	assertModuleExportNameOpt(propertyName)
	assertBindingIdentifier(name)
...
type ImportClause struct {
	...
	name          *BindingIdentifierNode // BindingIdentifierNode. Optional, default binding
}

func (f *NodeFactory) NewImportClause(isTypeOnly bool, name *BindingIdentifierNode, namedBindings *NamedImportBindings) *Node {
	assertBindingIdentifierOpt(name)
...
func (f *NodeFactory) NewNamespaceImport(name *BindingIdentifierNode) *Node {
	assertBindingIdentifier(name)
...
func (f *NodeFactory) NewNamespaceExportDeclaration(modifiers *ModifierListNode, name *BindingIdentifierNode) *Node {
...
func (f *NodeFactory) NewExportDeclaration(modifiers *ModifierListNode, isTypeOnly bool, exportClause *NamedExportBindings, moduleSpecifier *Expression, attributes *ImportAttributesNode) *Node {
	...
	assertExpressionOpt(moduleSpecifier)
	assertNodeOpt(attributes, isImportAttributes)
...
func (f *NodeFactory) NewConstructorDeclaration(modifiers *ModifierListNode, typeParameters *TypeParameterListNode, parameters []*ParameterDeclarationNode, returnType *TypeNode, body *FunctionBodyNode) *Node {
	assertModifiersOpt(modifiers)
	assertTypeParametersOpt(typeParameters)
	assertParameters(parameters)
	assertTypeNodeOpt(returnType)
...
func (f *NodeFactory) NewGetAccessorDeclaration(modifiers *ModifierListNode, name *PropertyName, typeParameters *TypeParameterListNode, parameters []*ParameterDeclarationNode, returnType *TypeNode, body *FunctionBodyNode) *Node {
	assertModifiersOpt(modifiers)
	assertPropertyName(name)
	assertTypeParametersOpt(typeParameters)
	assertParameters(parameters)
...
func (f *NodeFactory) NewSetAccessorDeclaration(modifiers *ModifierListNode, name *PropertyName, typeParameters *TypeParameterListNode, parameters []*ParameterDeclarationNode, returnType *TypeNode, body *FunctionBodyNode) *Node {
	assertModifiersOpt(modifiers)
	assertPropertyName(name)
	assertTypeParametersOpt(typeParameters)
	assertParameters(parameters)
...
func (f *NodeFactory) NewMethodSignatureDeclaration(modifiers *ModifierListNode, name *PropertyName, postfixToken *TokenNode, typeParameters *TypeParameterListNode, parameters []*ParameterDeclarationNode, returnType *TypeNode) *Node {
	assertModifiersOpt(modifiers)
	assertPropertyName(name)
	assertQuestionTokenOpt(postfixToken)
...
func (f *NodeFactory) NewMethodDeclaration(modifiers *ModifierListNode, asteriskToken *TokenNode, name *PropertyName, postfixToken *TokenNode, typeParameters *TypeParameterListNode, parameters []*ParameterDeclarationNode, returnType *TypeNode, body *FunctionBodyNode) *Node {
	assertModifiersOpt(modifiers)
	assertAsteriskTokenOpt(asteriskToken)
	assertPropertyName(name)
	assertQuestionTokenOpt(postfixToken)
...
type ClassStaticBlockDeclaration struct {
	...
	LocalsContainerBase
	ClassElementBase
	body *FunctionBodyNode // FunctionBodyNode
}

func (f *NodeFactory) NewClassStaticBlockDeclaration(modifiers *ModifierListNode, body *FunctionBodyNode) *Node {
	assertModifiersOpt(modifiers)
	assertFunctionBody(body)
...
func (f *NodeFactory) NewYieldExpression(asteriskToken *TokenNode, expression *Expression) *Node {
	assertAsteriskTokenOpt(asteriskToken)
	assertExpressionOpt(expression)
	assert(asteriskToken == nil || expression != nil, "If yield has an asterisk token it must have an expression")
...
func (f *NodeFactory) NewArrowFunction(modifiers *ModifierListNode, typeParameters *TypeParameterListNode, parameters []*ParameterDeclarationNode, returnType *TypeNode, equalsGreaterThanToken *TokenNode, body *FunctionBodyOrExpression) *Node {
	assertModifiersOpt(modifiers)
	assertTypeParametersOpt(typeParameters)
	assertParameters(parameters)
	assertTypeNodeOpt(returnType)
	assertToken(equalsGreaterThanToken, SyntaxKindEqualsGreaterThanToken)
	assertNode(body, isExpression, isFunctionBody)
	data := &ArrowFunction{}
	data.modifiers = modifiers
	data.typeParameters = typeParameters
	data.parameters = parameters
...
type FunctionExpression struct {
	...
	FunctionLikeWithBodyBase
	FlowNodeBase
	name           *BindingIdentifierNode // BindingIdentifierNode. Optional
	returnFlowNode *FlowNode
}

func (f *NodeFactory) NewFunctionExpression(modifiers *ModifierListNode, asteriskToken *TokenNode, name *BindingIdentifierNode, typeParameters *TypeParameterListNode, parameters []*ParameterDeclarationNode, returnType *TypeNode, body *FunctionBodyNode) *Node {
	assertModifiersOpt(modifiers)
	assertAsteriskTokenOpt(asteriskToken)
	assertBindingIdentifierOpt(name)
	assertTypeParametersOpt(typeParameters)
...
func (f *NodeFactory) NewConditionalExpression(condition *Expression, questionToken *TokenNode, whenTrue *Expression, colonToken *TokenNode, whenFalse *Expression) *Node {
	assertExpression(condition)
	assertToken(questionToken, SyntaxKindQuestionToken)
	assertExpression(whenTrue)
...
func (f *NodeFactory) NewPropertyAccessExpression(expression *Expression, questionDotToken *TokenNode, name *MemberName, flags NodeFlags) *Node {
	assertLeftHandSideExpression(expression)
	assertQuestionDotTokenOpt(questionDotToken)
	assertMemberName(name)
	assert(questionDotToken == nil || flags&NodeFlagsOptionalChain != 0, "A node with a '?.' token should be marked as an optional chain")
...
func (f *NodeFactory) NewElementAccessExpression(expression *Expression, questionDotToken *TokenNode, argumentExpression *Expression, flags NodeFlags) *Node {
	assertLeftHandSideExpression(expression)
	assertQuestionDotTokenOpt(questionDotToken)
	assertExpression(argumentExpression)
	assert(questionDotToken == nil || flags&NodeFlagsOptionalChain != 0, "A node with a '?.' token should be marked as an optional chain")
...
func (f *NodeFactory) NewCallExpression(expression *Expression, questionDotToken *TokenNode, typeArguments *TypeArgumentListNode, arguments []*Expression, flags NodeFlags) *Node {
	assertLeftHandSideExpression(expression)
	assertQuestionDotTokenOpt(questionDotToken)
	assertTypeArgumentsOpt(typeArguments)
...
type MetaProperty struct {
	...
	name         *IdentifierNameNode // IdentifierNameNode
}

func (f *NodeFactory) NewMetaProperty(keywordToken SyntaxKind, name *IdentifierNameNode) *Node {
	assertKind(keywordToken, SyntaxKindNewKeyword, SyntaxKindImportKeyword)
	assertIdentifierName(name)
	data := &MetaProperty{}
...
func (f *NodeFactory) NewTaggedTemplateExpression(tag *Expression, questionDotToken *TokenNode, typeArguments *TypeArgumentListNode, template *TemplateLiteral, flags NodeFlags) *Node {
	assertLeftHandSideExpression(tag)
	assertQuestionDotTokenOpt(questionDotToken)
	assertTypeArgumentsOpt(typeArguments)
...
func (f *NodeFactory) NewShorthandPropertyAssignment(modifiers *ModifierListNode, name *PropertyName, postfixToken *TokenNode, objectAssignmentInitializer *Expression) *Node {
	...
	assertPropertyName(name)
	assertTokenOpt(postfixToken, SyntaxKindQuestionToken, SyntaxKindExclamationToken)
	assertExpressionOpt(objectAssignmentInitializer)
...
func (f *NodeFactory) NewConditionalTypeNode(checkType *TypeNode, extendsType *TypeNode, trueType *TypeNode, falseType *TypeNode) *Node {
	assertTypeNode(checkType)
	assertTypeNode(extendsType)
	assertTypeNode(trueType)
	assertTypeNode(falseType)
...
func (f *NodeFactory) NewIndexedAccessTypeNode(objectType *TypeNode, indexType *TypeNode) *Node {
	assertTypeNode(objectType)
	assertTypeNode(indexType)
...
func (f *NodeFactory) NewTypePredicateNode(assertsModifier *TokenNode, parameterName *TypePredicateParameterName, typeNode *TypeNode) *Node {
	...
	assertNode(parameterName, isIdentifierReference, isThisTypeNode)
	assertTypeNodeOpt(typeNode)
...
func (f *NodeFactory) NewImportTypeNode(isTypeOf bool, argument *TypeNode, attributes *ImportAttributesNode, qualifier *EntityName, typeArguments *TypeArgumentListNode) *Node {
	assertTypeNode(argument)
	assertNodeOpt(attributes, isImportAttributes)
	assertEntityNameOpt(qualifier)
	assertTypeArgumentsOpt(typeArguments)
	data := &ImportTypeNode{}
...
func (f *NodeFactory) NewImportAttribute(name *ImportAttributeName, value *Expression) *Node {
	assertNode(name, isIdentifierName, isStringLiteral)
...
func (f *NodeFactory) NewNamedTupleTypeMember(dotDotDotToken *TokenNode, name *BindingIdentifierNode, questionToken *TokenNode, typeNode *TypeNode) *Node {
	assertDotDotDotTokenOpt(dotDotDotToken)
	assertBindingIdentifier(name)
	assertQuestionTokenOpt(questionToken)
	assertTypeNode(typeNode)
...
type JsxNamespacedName struct {
	ExpressionBase
	name      *IdentifierNameNode // IdentifierNameNode
	namespace *IdentifierNameNode // IdentifierNameNode
}

func (f *NodeFactory) NewJsxNamespacedName(name *IdentifierNameNode, namespace *IdentifierNameNode) *Node {
	assertIdentifierName(name)
	assertIdentifierName(namespace)
...
func (f *NodeFactory) NewJsxOpeningElement(tagName *JsxTagNameExpression, typeArguments *TypeArgumentListNode, attributes *JsxAttributesNode) *Node {
	assertJsxTagNameExpression(tagName)
	assertTypeArgumentsOpt(typeArguments)
...
func (f *NodeFactory) NewJsxAttribute(name *JsxAttributeName, initializer *JsxAttributeValue) *Node {
	assertNode(name, isIdentifierName, isJsxNamespacedName)
