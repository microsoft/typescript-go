...
func (p *Program) getJSSyntacticDiagnosticsForFile(sourceFile *ast.SourceFile) []*ast.Diagnostic {
	...
	p.walkNodeForJSDiagnostics(sourceFile, sourceFile.AsNode(), sourceFile.AsNode(), &diagnostics)

	p.jsDiagnosticCache.Store(sourceFile, diagnostics)
	return diagnostics
}

func (p *Program) walkNodeForJSDiagnostics(sourceFile *ast.SourceFile, node *ast.Node, parent *ast.Node, diags *[]*ast.Diagnostic) {
	if node == nil {
		return
	}
	...
	case ast.KindParameter, ast.KindPropertyDeclaration, ast.KindMethodDeclaration:
		...
		if parent.Kind == ast.KindParameter && parent.AsParameterDeclaration() != nil && parent.AsParameterDeclaration().QuestionToken == node {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.The_0_modifier_can_only_be_used_in_TypeScript_files, "?"))
			return
		}
		fallthrough
	case ast.KindMethodSignature, ast.KindConstructor, ast.KindGetAccessor, ast.KindSetAccessor,
		...
		if p.isTypeAnnotation(parent, node) {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.Type_annotations_can_only_be_used_in_TypeScript_files))
			return
		}
	}
	...
	case ast.KindImportClause:
		if node.AsImportClause() != nil && node.AsImportClause().IsTypeOnly {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, parent, diagnostics.X_0_declarations_can_only_be_used_in_TypeScript_files, "import type"))
			return
		}

	case ast.KindExportDeclaration:
		if node.AsExportDeclaration() != nil && node.AsExportDeclaration().IsTypeOnly {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.X_0_declarations_can_only_be_used_in_TypeScript_files, "export type"))
			return
		}

	case ast.KindImportSpecifier:
		if node.AsImportSpecifier() != nil && node.AsImportSpecifier().IsTypeOnly {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.X_0_declarations_can_only_be_used_in_TypeScript_files, "import...type"))
			return
		}
	...
	case ast.KindImportEqualsDeclaration:
		*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.X_import_can_only_be_used_in_TypeScript_files))
		return

	case ast.KindExportAssignment:
		if node.AsExportAssignment() != nil && node.AsExportAssignment().IsExportEquals {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.X_export_can_only_be_used_in_TypeScript_files))
			return
		}

	case ast.KindHeritageClause:
		if node.AsHeritageClause() != nil && node.AsHeritageClause().Token == ast.KindImplementsKeyword {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.X_implements_clauses_can_only_be_used_in_TypeScript_files))
			return
		}

	case ast.KindInterfaceDeclaration:
		*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.X_0_declarations_can_only_be_used_in_TypeScript_files, "interface"))
		return

	case ast.KindModuleDeclaration:
		moduleKeyword := "module"
		// For now, we'll just use "module" as the default since we don't have a flag to distinguish namespace vs module
		*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.X_0_declarations_can_only_be_used_in_TypeScript_files, moduleKeyword))
		return
	...
	case ast.KindEnumDeclaration:
		*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.X_0_declarations_can_only_be_used_in_TypeScript_files, "enum"))
		return

	case ast.KindNonNullExpression:
		*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.Non_null_assertions_can_only_be_used_in_TypeScript_files))
		return

	case ast.KindAsExpression:
		if node.AsAsExpression() != nil {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node.AsAsExpression().Type, diagnostics.Type_assertion_expressions_can_only_be_used_in_TypeScript_files))
			return
		}

	case ast.KindSatisfiesExpression:
		if node.AsSatisfiesExpression() != nil {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node.AsSatisfiesExpression().Type, diagnostics.Type_satisfaction_expressions_can_only_be_used_in_TypeScript_files))
			return
		}
	...
	case ast.KindConstructor, ast.KindMethodDeclaration, ast.KindFunctionDeclaration:
		...
		if p.isSignatureDeclaration(node) {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.Signature_declarations_can_only_be_used_in_TypeScript_files))
			return
		}
	}

	// Check for type parameters, type arguments, and modifiers
	p.checkTypeParametersAndModifiers(sourceFile, node, diags)

	// Recursively walk children
	node.ForEachChild(func(child *ast.Node) bool {
		p.walkNodeForJSDiagnostics(sourceFile, child, node, diags)
		return false
	})
}
...
func (p *Program) checkTypeParametersAndModifiers(sourceFile *ast.SourceFile, node *ast.Node, diags *[]*ast.Diagnostic) {
	...
	if p.hasTypeParameters(node) {
		*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.Type_parameter_declarations_can_only_be_used_in_TypeScript_files))
	}

	// Check type arguments
	if p.hasTypeArguments(node) {
		*diags = append(*diags, p.createDiagnosticForNode(sourceFile, node, diagnostics.Type_arguments_can_only_be_used_in_TypeScript_files))
	}

	// Check modifiers
	p.checkModifiers(sourceFile, node, diags)
}
...
func (p *Program) checkModifiers(sourceFile *ast.SourceFile, node *ast.Node, diags *[]*ast.Diagnostic) {
	...
	case ast.KindVariableStatement:
		if node.AsVariableStatement() != nil && node.AsVariableStatement().Modifiers() != nil {
			p.checkModifierList(sourceFile, node.AsVariableStatement().Modifiers(), true, diags)
		}
	case ast.KindPropertyDeclaration:
		if node.AsPropertyDeclaration() != nil && node.AsPropertyDeclaration().Modifiers() != nil {
			p.checkPropertyModifiers(sourceFile, node.AsPropertyDeclaration().Modifiers(), diags)
		}
	case ast.KindParameter:
		if node.AsParameterDeclaration() != nil && node.AsParameterDeclaration().Modifiers() != nil {
			p.checkParameterModifiers(sourceFile, node.AsParameterDeclaration().Modifiers(), diags)
		}
	}
}

func (p *Program) checkModifierList(sourceFile *ast.SourceFile, modifiers *ast.ModifierList, isConstValid bool, diags *[]*ast.Diagnostic) {
	if modifiers == nil {
		return
	}
	...
	for _, modifier := range modifiers.Nodes {
		...
		p.checkModifier(sourceFile, modifier, isConstValid, diags)
	}
}

func (p *Program) checkPropertyModifiers(sourceFile *ast.SourceFile, modifiers *ast.ModifierList, diags *[]*ast.Diagnostic) {
	if modifiers == nil {
		return
	}
	...
	for _, modifier := range modifiers.Nodes {
		...
		default:
			if p.isTypeScriptOnlyModifier(modifier) {
				*diags = append(*diags, p.createDiagnosticForNode(sourceFile, modifier, diagnostics.The_0_modifier_can_only_be_used_in_TypeScript_files, p.getTokenText(modifier)))
			}
		}
	}
}

func (p *Program) checkParameterModifiers(sourceFile *ast.SourceFile, modifiers *ast.ModifierList, diags *[]*ast.Diagnostic) {
	if modifiers == nil {
		return
	}
	...
	for _, modifier := range modifiers.Nodes {
		...
		if p.isTypeScriptOnlyModifier(modifier) {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, modifier, diagnostics.Parameter_modifiers_can_only_be_used_in_TypeScript_files))
		}
	}
}

func (p *Program) checkModifier(sourceFile *ast.SourceFile, modifier *ast.Node, isConstValid bool, diags *[]*ast.Diagnostic) {
	switch modifier.Kind {
	case ast.KindConstKeyword:
		if !isConstValid {
			*diags = append(*diags, p.createDiagnosticForNode(sourceFile, modifier, diagnostics.The_0_modifier_can_only_be_used_in_TypeScript_files, "const"))
		}
...
func (p *Program) createDiagnosticForNode(sourceFile *ast.SourceFile, node *ast.Node, message *diagnostics.Message, args ...any) *ast.Diagnostic {
	return ast.NewDiagnostic(sourceFile, p.getErrorRangeForNode(sourceFile, node), message, args...)
}

func (p *Program) getErrorRangeForNode(sourceFile *ast.SourceFile, node *ast.Node) core.TextRange {
	if node == nil {
