...
func (f *isolatedDeclarationsFixer) createNamespaceForExpandoProperties(expandoFunc *ast.Node) string {
	...
	for _, symbol := range elements {
		...
		varDeclList := factory.NewVariableDeclarationList(ast.NodeFlagsNone, factory.NewNodeList([]*ast.Node{varDecl}))
...
func (f *isolatedDeclarationsFixer) extractAsVariable(span core.TextRange) string {
	...
	if parentPropertyAssignment != nil {
		...
		varDeclList := factory.NewVariableDeclarationList(ast.NodeFlagsConst, factory.NewNodeList([]*ast.Node{varDecl}))
...
func (f *isolatedDeclarationsFixer) transformExportAssignment(defaultExport *ast.Node) string {
	...
	newExport := factory.UpdateExportAssignment(defaultExport.AsExportAssignment(), defaultExport.Modifiers(), nil, defaultIdentifier.AsNode())
	...
	if ast.IsObjectBindingPattern(bindingPattern) {
		for _, element := range bindingPattern.AsBindingPattern().Elements.Nodes {
			...
			if be.PropertyName != nil && ast.IsComputedPropertyName(be.PropertyName) {
				...
				compVarDeclList := factory.NewVariableDeclarationList(ast.NodeFlagsConst, factory.NewNodeList([]*ast.Node{compVarDecl}))
...
func (f *isolatedDeclarationsFixer) emitBindingElementVariable(
	...
	if be.Initializer != nil {
		...
		tempVarDeclList := factory.NewVariableDeclarationList(ast.NodeFlagsConst, factory.NewNodeList([]*ast.Node{tempVarDecl}))
...
func (f *isolatedDeclarationsFixer) inferType(node *ast.Node, variableType *checker.Type) *ast.TypeNode {
	...
	if ast.IsParameter(node) && f.checker.RequiresAddingImplicitUndefined(node) {
...
func typeParamHasDefault(tp *checker.Type) bool {
	...
	for _, decl := range sym.Declarations {
		if ast.IsTypeParameterDeclaration(decl) && decl.AsTypeParameter().DefaultType != nil {
			return true
		}
	}
	return false
