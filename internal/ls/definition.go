...
func (l *LanguageService) ProvideDefinition(ctx context.Context, documentURI lsproto.DocumentUri, position lsproto.Position) (*lsproto.LocationOrLocationsOrDefinitionLinksOrNull, error) {
...
func (l *LanguageService) ProvideTypeDefinition(ctx context.Context, documentURI lsproto.DocumentUri, position lsproto.Position) (*lsproto.LocationOrLocationsOrDefinitionLinksOrNull, error) {
	program, file := l.getProgramAndFile(documentURI)
	node := astnav.GetTouchingPropertyName(file, int(l.converters.LineAndCharacterToPosition(file, position)))
	if node.Kind == ast.KindSourceFile {
		return nil, nil
...
func (l *LanguageService) createLocationsFromDeclarations(declarations []*ast.Node) *lsproto.LocationOrLocationsOrDefinitionLinksOrNull {
...
func (l *LanguageService) createLocationFromFileAndRange(file *ast.SourceFile, textRange core.TextRange) *lsproto.LocationOrLocationsOrDefinitionLinksOrNull {
