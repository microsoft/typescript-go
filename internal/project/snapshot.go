	...
	for _, project := range newSnapshot.ProjectCollection.Projects() {
		if project.Program != nil {
			if project.ProgramLastUpdate == newSnapshotID {
				// New program created during this snapshot gets unconditionally added to the program counter.
				...
				// Reused program gets incremented in the counter only if it still exists in the counter.
				// Otherwise, it's already been disposed (the snapshot we're cloning must have been
				// concurrently disposed), and re-adding it would eventually cause a double-dispose.
