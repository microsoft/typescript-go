...
func TestProjectProgramUpdateKind(t *testing.T) {
	...
	t.Run("NewFiles on initial build", func(t *testing.T) {
		...
		_, _, err := session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///src/index.ts"))
	...
	t.Run("NewFiles on root addition", func(t *testing.T) {
		...
		_, _, err = session.GetLanguageService(context.Background(), lsproto.DocumentUri("file:///src/newfile.ts"))
		assert.NilError(t, err)
		snapshot, release := session.Snapshot()
		defer release()
		configured := snapshot.ProjectCollection.ConfiguredProject(tspath.Path("/src/tsconfig.json"))
...
func TestProject(t *testing.T) {
	...
	t.Run("commandLineWithTypingsFiles is reset on CommandLine change", func(t *testing.T) {
		...
		_, _, err := session.GetLanguageService(context.Background(), uri1)
		assert.NilError(t, err)
		...
		_, _, err = session.GetLanguageService(context.Background(), uri2)
		assert.NilError(t, err)
	})
}
