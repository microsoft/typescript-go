...
type Service struct {
	...
	// !!! sheetal to make sure this doesnt need lock - watch events, Ti events and others
	// !!! project.close to verify on that same note as well
...
func (s *Service) InferredProject(rootPath tspath.Path) *Project {
	...
	if s.unrootedInferredProject != nil && s.unrootedInferredProject.rootPath == rootPath {
...
func (s *Service) removeProject(project *Project) {
	s.Log("`remove Project::")
	project.print( /*writeProjectFileNames*/ true /*writeFileExplaination*/, true /*writeFileVersionAndText*/, false, &strings.Builder{})
