...
type ProjectHost interface {
	...
	GetResolvedProjectReference(fileName string, path tspath.Path, forProject tspath.Path) *tsoptions.ParsedCommandLine
	ReleaseResolvedProjectReference(path tspath.Path, forProject tspath.Path)
...
func (p *Project) GetResolvedProjectReference(fileName string, path tspath.Path) *tsoptions.ParsedCommandLine {
	return p.host.GetResolvedProjectReference(fileName, path, p.configFilePath)
}
...
func (p *Project) updateGraph() (*compiler.Program, bool) {
	...
	if p.kind == KindConfigured && p.pendingReload != PendingReloadNone {
		...
		case PendingReloadFileNames:
			p.parsedCommandLine = p.host.GetResolvedProjectReference(p.configFileName, p.configFilePath, p.configFilePath)
		...
		if oldProgram != nil {
			...
			oldProgram.ForEachResolvedProjectReference(func(path tspath.Path, ref *tsoptions.ParsedCommandLine) bool {
				if _, ok := p.program.GetResolvedProjectReferenceFor(path); !ok {
					p.host.ReleaseResolvedProjectReference(path, p.configFilePath)
				}
				return true
			})
		}
...
func (p *Project) Close() {
	...
	if p.program != nil {
		...
		if p.kind == KindConfigured {
			p.host.ReleaseResolvedProjectReference(p.configFilePath, p.configFilePath)
		}
		p.program = nil
	}
