...
func TestRefCountingCaches(t *testing.T) {
	...
	t.Run("parseCache", func(t *testing.T) {
		...
		t.Run("reuse unchanged file", func(t *testing.T) {
			...
			ls, _, err := session.GetLanguageService(context.Background(), "file:///user/username/projects/myproject/src/main.ts")
		...
		t.Run("release file on close", func(t *testing.T) {
			...
			_, _, err := session.GetLanguageService(context.Background(), "file:///user/username/projects/myproject/src/utils.ts")
			assert.NilError(t, err)
			assert.Equal(t, utilsEntry.refCount, 1)
