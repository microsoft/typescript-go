...
func main() {
	...
	if *diagnosticsOutput == "" || *locOutput == "" {
		flag.Usage()
		return
	}
	...
	locBuf := generateLocalizations(knownKeys)
...
func generateLocalizations(knownKeys map[string]bool) *bytes.Buffer {
	var buf bytes.Buffer
	...
	// Generate supported tags and matcher first
	...
	// Now generate the actual locale translation tables
	for _, loc := range locales {
		fmt.Fprintf(&buf, "\nvar %s = sync.OnceValue(func() map[Key]string {\n", loc.varName)
		fmt.Fprintf(&buf, "m := make(map[Key]string, %d)\n", len(loc.messages))

		// Sort keys for consistent output
		keys := slices.Collect(maps.Keys(loc.messages))
		slices.Sort(keys)

		for _, key := range keys {
			fmt.Fprintf(&buf, "m[%q] = %q\n", key, loc.messages[key])
		}
