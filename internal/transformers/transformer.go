...
func getModuleTransformer(emitContext *printer.EmitContext, options *core.CompilerOptions, resolver binder.ReferenceResolver) *Transformer {
	switch options.GetEmitModuleKind() {
	case core.ModuleKindPreserve:
		// `ESModuleTransformer` contains logic for preserving CJS input syntax in `--module preserve`
		return NewESModuleTransformer(emitContext, options, resolver)
	...
	default:
		return NewCommonJSModuleTransformer(emitContext, options, resolver)
	}
}
...
func GetScriptTransformers(emitContext *printer.EmitContext, host printer.EmitHost, sourceFile *ast.SourceFile) []*Transformer {
	...
	tx = append(tx, getModuleTransformer(emitContext, options, referenceResolver))
	return tx
}
