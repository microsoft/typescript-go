...
func startTracingIfNeeded(sys tsc.System, config *tsoptions.ParsedCommandLine, testing tsc.CommandLineTesting) *tracing.Tracing {
	...
	var clock tracing.Clock
	if testing != nil {
		clock = tracing.NewMonotonicClock(1) // 1 microsecond per call
	}
	tr, err := tracing.StartTracingWithClock(sys.FS(), traceDir, configFilePath, clock)
	if err != nil {
