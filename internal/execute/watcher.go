...
func (w *Watcher) start() {
	w.host = compiler.NewCompilerHost(w.sys.GetCurrentDirectory(), w.sys.FS(), w.sys.DefaultLibraryPath(), nil, w.sys.PnpApi(), getTraceFromSys(w.sys, w.config.Locale(), w.testing))
...
func (w *Watcher) hasErrorsInTsConfig() bool {
	...
	w.host = compiler.NewCompilerHost(w.sys.GetCurrentDirectory(), w.sys.FS(), w.sys.DefaultLibraryPath(), extendedConfigCache, w.sys.PnpApi(), getTraceFromSys(w.sys, w.config.Locale(), w.testing))
	return false
}
