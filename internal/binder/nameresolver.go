...
type NameResolver struct {
	...
	ResolveAlias                       func(symbol *ast.Symbol) *ast.Symbol
	GetMergedSymbol                    func(symbol *ast.Symbol) *ast.Symbol
	GetExportsOfSymbol                 func(symbol *ast.Symbol) ast.SymbolTable
	...
	OnFailedToResolveQualifiedName     func(name *ast.Node, namespace *ast.Symbol, right *ast.Node, meaning ast.SymbolFlags)
	OnSuccessfullyResolvedEntityName   func(name *ast.Node, symbol *ast.Symbol)
	GetCannotFindNameDiagnosticForName func(node *ast.Node) *diagnostics.Message
}
...
loop:
	for location != nil {
		...
		data := location.LocalsContainerData()
		if data != nil {
			locals := data.Locals
			...
			if locals != nil && !ast.IsGlobalSourceFile(location) {
				...
				if result != nil {
					useResult := true
					if ast.IsFunctionLike(location) && lastLocation != nil && lastLocation != ast.GetBodyOfNode(location) {
...
func (r *NameResolver) useOuterVariableScopeInParameter(result *ast.Symbol, location *ast.Node, lastLocation *ast.Node) bool {
	if ast.IsParameter(lastLocation) {
		body := ast.GetBodyOfNode(location)
	...
	case ast.KindIdentifier:
		...
		if !ignoreErrors {
			if meaning == ast.SymbolFlagsNamespace || ast.NodeIsSynthesized(name) {
				...
				message = r.getCannotFindNameDiagnosticForName(ast.GetFirstIdentifier(name))
			}
		}
		...
		symbol = r.getMergedSymbol(r.Resolve(resolveLocation, name.AsIdentifier().Text, meaning, message, true /*isUse*/, false /*excludeGlobals*/))
	case ast.KindQualifiedName:
		qualified := name.AsQualifiedName()
		symbol = r.resolveQualifiedName(name, qualified.Left, qualified.Right, meaning, ignoreErrors, dontResolveAlias, location)
	case ast.KindPropertyAccessExpression:
		access := name.AsPropertyAccessExpression()
		symbol = r.resolveQualifiedName(name, access.Expression, access.Name(), meaning, ignoreErrors, dontResolveAlias, location)
	default:
		panic("Unknown entity name kind")
	}
	if symbol != nil {
		if r.OnSuccessfullyResolvedEntityName != nil {
			r.OnSuccessfullyResolvedEntityName(name, symbol)
		}
		if symbol.Flags&meaning == 0 && !dontResolveAlias {
			return r.resolveAlias(symbol)
		}
	}
	return symbol
}

func (r *NameResolver) resolveQualifiedName(name *ast.Node, left *ast.Node, right *ast.Node, meaning ast.SymbolFlags, ignoreErrors bool, dontResolveAlias bool, location *ast.Node) *ast.Symbol {
	namespace := r.ResolveEntityName(left, ast.SymbolFlagsNamespace, ignoreErrors /*dontResolveAlias*/, false, location)
	if namespace == nil || ast.NodeIsMissing(right) {
		return nil
	}
	if namespace == r.UnknownSymbol {
		return namespace
	}
	text := right.AsIdentifier().Text
	symbol := r.getMergedSymbol(r.getSymbol(r.getExportsOfSymbol(namespace), text, meaning))
	if symbol != nil && namespace.Flags&ast.SymbolFlagsAlias != 0 {
		...
		symbol = r.getMergedSymbol(r.getSymbol(r.getExportsOfSymbol(r.resolveAlias(namespace)), text, meaning))
	}
	if symbol == nil {
		if !ignoreErrors && r.OnFailedToResolveQualifiedName != nil {
			r.OnFailedToResolveQualifiedName(name, namespace, right, meaning)
		}
	}
	return symbol
}

func (r *NameResolver) resolveAlias(symbol *ast.Symbol) *ast.Symbol {
	if r.ResolveAlias != nil {
		return r.ResolveAlias(symbol)
	}
	return r.UnknownSymbol
}

func (r *NameResolver) getCannotFindNameDiagnosticForName(node *ast.Node) *diagnostics.Message {
	if r.GetCannotFindNameDiagnosticForName != nil {
		return r.GetCannotFindNameDiagnosticForName(node)
	}
	return diagnostics.Cannot_find_name_0
}

func (r *NameResolver) getMergedSymbol(symbol *ast.Symbol) *ast.Symbol {
	if r.GetMergedSymbol != nil {
		return r.GetMergedSymbol(symbol)
	}
	return symbol
}

func (r *NameResolver) getSymbol(symbols ast.SymbolTable, name string, meaning ast.SymbolFlags) *ast.Symbol {
	return r.Lookup(symbols, name, meaning)
}

func (r *NameResolver) getExportsOfSymbol(symbol *ast.Symbol) ast.SymbolTable {
	if r.GetExportsOfSymbol != nil {
		return r.GetExportsOfSymbol(symbol)
	}
