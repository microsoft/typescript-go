...
func generateLibs() {
	...
	embedGenerated.WriteString("func readEmbeddedFile(path string) (string, bool) {\n")
	embedGenerated.WriteString("\tswitch path {\n")
	for _, lib := range libs {
		varName := strings.ReplaceAll(lib.target, ".", "_")
		embedGenerated.WriteString("\tcase \"libs/" + lib.target + "\":\n")
		embedGenerated.WriteString("\t\treturn " + varName + ", true\n")
	}
	embedGenerated.WriteString("\t}\n")
	embedGenerated.WriteString("\treturn \"\", false\n")
	embedGenerated.WriteString("}\n\n")

	for _, lib := range libs {
		varName := strings.ReplaceAll(lib.target, ".", "_")
		embedGenerated.WriteString("//go:embed libs/" + lib.target + "\n")
		embedGenerated.WriteString("var " + varName + " string\n\n")
	}
