...
const (
	...
	NodeFlagsHasAsyncFunctions               NodeFlags = 1 << 10 // If the file has async functions (initialized by binding)
	NodeFlagsDisallowInContext               NodeFlags = 1 << 11 // If node was parsed in a context where 'in-expressions' are not allowed
	NodeFlagsYieldContext                    NodeFlags = 1 << 12 // If node was parsed in the 'yield' context created when parsing a generator
	NodeFlagsDecoratorContext                NodeFlags = 1 << 13 // If node was parsed as part of a decorator
	NodeFlagsAwaitContext                    NodeFlags = 1 << 14 // If node was parsed in the 'await' context created when parsing an async function
	NodeFlagsDisallowConditionalTypesContext NodeFlags = 1 << 15 // If node was parsed in a context where conditional types are not allowed
	NodeFlagsThisNodeHasError                NodeFlags = 1 << 16 // If the parser encountered an error when parsing the code that created this node
	NodeFlagsJavaScriptFile                  NodeFlags = 1 << 17 // If node was parsed in a JavaScript
	NodeFlagsThisNodeOrAnySubNodesHasError   NodeFlags = 1 << 18 // If this node or any of its children had an error
	NodeFlagsHasAggregatedChildData          NodeFlags = 1 << 19 // If we've computed data from children and cached it in this node
	...
	NodeFlagsHasJSDoc        NodeFlags = 1 << 22 // If node has preceding JSDoc comment(s)
	NodeFlagsJSDoc           NodeFlags = 1 << 23 // If node was parsed inside jsdoc
	NodeFlagsAmbient         NodeFlags = 1 << 24 // If node was inside an ambient context -- a declaration file, or inside something with the `declare` modifier.
	NodeFlagsInWithStatement NodeFlags = 1 << 25 // If any ancestor of node was the `statement` of a WithStatement (not the `expression`)
	NodeFlagsJsonFile        NodeFlags = 1 << 26 // If node was parsed in a Json
	NodeFlagsDeprecated      NodeFlags = 1 << 27 // If has '@deprecated' JSDoc tag
