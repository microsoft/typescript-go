...
func (g *NameGenerator) makeTempVariableName(flags tempFlags, reservedInNestedScopes bool, privateName bool, prefix string, suffix string) string {
	...
	if simple {
		temp = g.getTempFlags(privateName)
	} else {
		...
		temp = g.getTempFlagsForFormattedName(privateName, key)
	}

	if flags != 0 && temp&flags == 0 {
		...
		if g.isUniqueName(fullName, privateName) {
			...
			if simple {
				g.setTempFlags(privateName, temp)
			} else {
				g.setTempFlagsForFormattedName(privateName, key, temp)
			}
			return fullName
		}
	}
