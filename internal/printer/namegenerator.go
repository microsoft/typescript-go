...
func (g *NameGenerator) makeTempVariableName(flags tempFlags, reservedInNestedScopes bool, privateName bool, prefix string, suffix string) string {
	var tempFlagsValue tempFlags
	var key string
	simple := len(prefix) == 0 && len(suffix) == 0
	if simple {
		tempFlagsValue = g.getTempFlags(privateName)
	} else {
		...
		tempFlagsValue = g.getTempFlagsForFormattedName(privateName, key)
	}

	if flags != 0 && tempFlagsValue&flags == 0 {
		fullName := formatGeneratedName(privateName, prefix, "_i", suffix)
		if g.isUniqueName(fullName, privateName) {
			tempFlagsValue |= flags
			g.reserveName(fullName, privateName, reservedInNestedScopes, true /*temp*/)
			if simple {
				g.setTempFlags(privateName, tempFlagsValue)
			} else {
				g.setTempFlagsForFormattedName(privateName, key, tempFlagsValue)
			}
			return fullName
		}
	}
	...
	for {
		count := tempFlagsValue & tempFlagsCountMask
		tempFlagsValue++
		// Skip over 'i'
		if count != 8 {
			var name string
