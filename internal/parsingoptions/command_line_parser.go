...
type CommandLineOptionBase struct { //???
	...
	description               *ast.Diagnostic           //DM			                       // The message describing what the command line switch does.
	...
	paramType                  *ast.Diagnostic //DM                       // The name to be used for a non-boolean option's parameter
...
func convertConfigFileToObject(
	...
	if len(sourceFile.Statements) > 0 { //check
		rootExpression = sourceFile.Statements[0].AsExpressionStatement().Expression
	}
	if rootExpression != nil && rootExpression.Kind != ast.KindObjectLiteralExpression {
		...
		if ast.IsArrayLiteralExpression(rootExpression) {
			var firstObject = core.Find(rootExpression.AsArrayLiteralExpression().Elements, ast.IsObjectLiteralExpression)
			if firstObject != nil {
...
// 		rootExpression = *sourceFile.Statements[0].Expression()
// 	}
...
// 	convertedElements := make([]ast.Node, len(elements))
// 	for i, element := range elements {
// 		convertedElements[i] = convertPropertyValueToJson(element, elementOption).(ast.Node)
// 	}
// 	// filteredElements := core.Filter(convertedElements, func(v ast.Node) bool {
// 	// 	return v != nil
// 	// })
...
func convertObjectLiteralExpressionToJson(
	...
	for _, element := range node.Properties {
		...
		//for now
		keyText = "exclude"
		var value = convertPropertyValueToJson(element, nil, returnValue, jsonConversionNotifier) // this needs to be element.initializer need to come back
...
func convertPropertyValueToJson(valueExpression *ast.Expression, option *tsConfigOnlyOption, returnValue bool, jsonConversionNotifier *JsonConversionNotifier) any {
	...
	case ast.KindArrayLiteralExpression:
		...
		// 	(valueExpression.AsArrayLiteralExpression()).elements,
		// 	option && (option.AsCommandLineOptionOfListType()).element,
		// 	returnValue,
		// )
		//return result
	}
