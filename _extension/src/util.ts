...
import { getNodeModulesCandidates } from "./tsdkPackage";
...
export async function resolveTsdkPathToExe(tsdkPath: string): Promise<ExeInfo | undefined> {
    ...
    for (const packagePath of [resolved, vscode.Uri.joinPath(resolved, "..")]) {
        try {
            ...
            for (const nodeModules of getNodeModulesCandidates(packagePath.fsPath, name)) {
                const exePath = vscode.Uri.joinPath(vscode.Uri.file(nodeModules), "@typescript", platformPackage, "lib", exeName);
                try {
                    await vscode.workspace.fs.stat(exePath);
