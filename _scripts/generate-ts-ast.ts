...
// TS members only (filter Go-only/inherited tsOnly issues)
function tsMembers(node: NodeType): MemberInfo[] {
    return node.members.filter(member => !member.isKindParam() && !member.goOnly);
}

function isInheritedFromTsBase(member: MemberInfo): boolean {
    return member.inherited && !!member.inheritedField && !member.inheritedField.goOnly;
}
...
// Members excluded from factory function parameters (schema-driven).
function isFactoryExcluded(m: MemberInfo): boolean {
    // NodeFlags members are set on node.Flags, not factory data
    const memberType = m.type;
    return memberType.kind === "primitive" && memberType.name === "NodeFlags";
}

// Get factory members in correct order: child members first, data members last
function factoryMembers(node: NodeType): MemberInfo[] {
    const members = tsMembers(node).filter(m => !isFactoryExcluded(m));
    const children = members.filter(m => m.isChild());
    const data = members.filter(m => !m.isChild());
...
function syntaxKindChecksForNode(node: NodeType): string[] {
    return node.kindTypes().map(kind => `SyntaxKind.${kind.name}`);
}
...
function generateFactory(): string {
    ...
    // Always needed
    for (const t of ["Node", "NodeArray", "KeywordTypeSyntaxKind", "Token", "SourceFile", "KeywordTypeNode", "EndOfFile", "ImportPhaseModifierSyntaxKind", "Statement"]) {
        importTypes.add(t);
    }
...
//`);
    ...
    for (const name of sortedGetters) {
        out.push(`    get ${name}() { return this._data?.${name}; }`);
    }
    out.push(``);
    ...
    for (const node of api.nodes()) {
        ...
        const syntaxKinds = [node.syntaxKindName, ...node.kindAliases];
        for (const sk of syntaxKinds) {
            out.push(`        case SyntaxKind.${sk}:`);
        }
    ...
    for (const node of api.nodes()) {
        ...
        const visits = childMembers.map(m => {
            ...
            if (listKind === "NodeList" || listKind === "raw") {
        ...
        for (const sk of syntaxKinds) {
            out.push(`    [SyntaxKind.${sk}]: (data, cbNode, cbNodes) =>`);
            out.push(`        ${body},`);
        }
    }
    // Variant nodes
    ...
    for (const node of concreteFactoryNodes) {
        ...
        for (const m of members) {
            ...
            const opt = m.optional;
            const memberType = m.type;
            if (!m.isChild() && memberType.kind === "primitive" && memberType.name === "bool") {
    ...
    for (const node of concreteFactoryNodes) {
        ...
        for (const m of updateChildMembers) {
            const propName = api.uncapitalize(m.name);
            const paramName = tsParamName(propName);
            const paramType = tsParamTypeFrom(m.type);
            if (m.optional) {
...
function generateVisitor(): string {
    ...
    out.push(`// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!`);
    out.push(`// !!! THIS FILE IS AUTO-GENERATED - DO NOT EDIT !!!`);
    out.push(`// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!`);
    out.push(`//`);
    out.push(`// Source: _scripts/ast.json`);
    out.push(`// Generator: _scripts/generate-ts-ast.ts`);
    out.push(`//`);
    out.push("");
    out.push(`import { SyntaxKind } from "#enums/syntaxKind";`);
    out.push(`import type {`);
...
function main() {
    ...
    const visitorPath = path.join(ROOT, "_packages/ast/src/visitor.ts");
